...
estimatedTL %>% 
  ggplot() +
  geom_boxplot(aes(x = factor(Treatment), y = Aug_28_estimate, fill = Treatment, color =Treatment)) +
  theme_classic() + 
...
First clean a little and add final TRBI data
...
metadat %>% 
  ...
  mutate(Coral_vol = 4/3*pi*(0.5*Height_cm)*(0.5*Longest_length_cm)*(0.5*Perpendicular_to_longest_length_cm)) -> metadatint
...
metadatint %>% 
  ...
  mutate(TRBI_abundance = replace_na(TRBI_abundance, 0)) -> metadat9
...
```{r}
m.como <- glm(Abundance ~ Crab + Nutrients + Crab:Nutrients + Coral_vol, 
...
m.como <- glm(Abundance ~ Crab + Nutrients + Coral_vol + Crab:Nutrients, family = poisson, data = tst)
  summary(m.como)
```
...
```{r}
new.data <- comos_count[,c("Crab", "Nutrients")]
new.data$Coral_vol <- mean(comos_count$Coral_vol)
...
```{r}
m.comoTRB <- glm(Abundance ~ TRBI_abundance + Coral_vol, 
              family = poisson, data = comos_count)
  summary(m.comoTRB)
...
```{r}
new.data <- dafls_count[,c("Crab", "Nutrients")]
new.data$Coral_vol <- mean(dafls_count$Coral_vol)
...
m.lil.dafl <- glm(Wittle_abundance ~ Crab + Nutrients + Coral_vol, family = poisson, data = tst)
  summary(m.lil.dafl)
```
...
```{r}
m.big.dafl <- glm(Big_abundance ~ Crab + Nutrients + Coral_vol, family = poisson, data = big_dafls)
  summary(m.big.dafl) # Hmmm interesting there actually is an effect of nutrients and volume
...
```{r}
m.daflsize <- glm(cafi_size_mm ~ Coral_vol + Nutrients + Coral_vol:Nutrients, data = dafl_uncount, family = gaussian)
  summary(m.daflsize)
```
...
```{r}
m.palaemonid <- glm(Abundance ~ Crab + Nutrients + Nutrients:Crab + Coral_vol, family = poisson, data =  palaemonids_count)
...
m.palaemonid <- glm(Abundance ~ Crab + Nutrients + Coral_vol + Crab:Nutrients, family = poisson, data = tst)
  summary(m.palaemonid) # hmmm 40 was really influential here
...
```{r}
new.data <- palaemonids_count[,c("Crab", "Nutrients")] 
new.data <- cbind(new.data, Coral_vol = mean(palaemonids_count$Coral_vol))
...
```{r}
mtst <- glm(Palaemonid_abund ~  TRBI_abundance + Coral_vol, family = poisson, data = alphPalaeTRBI)
  summary(mtst)
```
...
```{r}
m.galatheid <- glm(Abundance ~ Crab + Nutrients + Nutrients:Crab + Coral_vol, family = poisson, data =  galatheids_count)
  summary(m.galatheid)  
...
```{r}
new.data <- galatheids_count[,c("Crab", "Nutrients")] 
new.data <- cbind(new.data, Coral_vol = mean(galatheids_count$Coral_vol))
...
trapeziids.G %>% 
  ...
  left_join(metadat9, by = "coral_id") -> gravidTraps
```
...
```{r}
m.trapeziid <- glm(Abundance ~ Crab + Nutrients +  Coral_vol, family = poisson, data
                   =  trapeziids_count)
...
```{r}
new.data <- trapeziids_count[,c("Crab", "Nutrients")] 
new.data <- cbind(new.data, Coral_vol = mean(trapeziids_count$Coral_vol))
...
```{r}
m.dafl.biom <- glm(Biomass ~ Crab + Nutrients + Coral_vol,
                   data = dafls_biomass)
  summary(m.dafl.biom)
```
...
```{r}
m.como.biom <- glm(Biomass ~ Crab + Nutrients + Coral_vol,
                   data = comos_biomass)
...
```{r}
m.alph.biom <- glm(Biomass ~ Crab + Nutrients + Coral_vol,
                   data = alph_biomass)
  summary(m.alph.biom) # still an effect of crab
```
...
```{r}
m.gama.biom <- glm(Biomass ~ Crab + Nutrients + Crab:Nutrients + Coral_vol,
                   data = gamas_biomass)
...
```{r}
m.comoAg <- glm(Abundance ~ Nutrients + MeanAggr + MeanAggr:Nutrients + Coral_vol, 
...
pred$PredAbund <- predict(m.comoAg, 
                 pred, 
                 type = "response")
...
```{r}
m.comoAg33 <- glm(Abundance ~ Nutrients + MeanAggr + Coral_vol, 
...
comoAgg %>% 
  filter(Crab == "Y") %>% 
  filter(TRBI_abundance > 0) -> comoAggTRBI

m.comoAgTRB <- glm(Abundance ~ Nutrients + MeanAggr + Coral_vol, 
...
```{r}
m.daflAg <- glm(Abundance ~ Nutrients + Crab + MeanAggr + MeanAggr:Nutrients + Coral_vol, 
...
m.daflAgCR <- glm(Abundance ~ Nutrients + MeanAggr + Nutrients:MeanAggr + Coral_vol, 
...
cafi %>% 
  filter(search_term == "Trapezia bidentata") %>% 
  filter(cafi_size_mm >= 7.5) %>% 
  left_join(metadat9, by = "coral_id") -> lg_TRBI
```
